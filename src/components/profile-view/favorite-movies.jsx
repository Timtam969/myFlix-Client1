...
export default function FavoriteMovies(props) {
  ...
  const getUser = (response) => {
    axios
      .get(`https://melsflix.herokuapp.com/users/${currentUser}`, {
  ...
  const removeFavorite = (movieId) => {
    axios
      .delete(`https://melsflix.herokuapp.com/users/${currentUser}/movies/${movieId}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((response) => {
        setFavoriteMovies(response.data.favoriteMovies);
        ...
        <Row>
          ...
          {!favoriteMovies ? (
            ...
            favoriteMovies.map((movieId) => {
              ...
              return (
                <Col xs={12} md={6} lg={4} className="fav-movie" key={movie._id}>
                  <Figure>
